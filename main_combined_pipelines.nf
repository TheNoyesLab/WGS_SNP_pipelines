...
process RunFastqConvert {
    ...
    output:
      ...
      file("interleaved_reads/${sample_id}.fasta") into (fasta_files)

    """
    shuffleSplitReads.pl --numcpus 8 -o interleaved_reads/ *_{1,2}.fastq.gz
    cd interleaved_reads/
    cp ${sample_id}.fastq.gz ${sample_id}.cp.fastq.gz
    zcat ${sample_id}.cp.fastq.gz | paste - - - - | sed 's/^@/>/g'| cut -f1-2 | tr '\t' '\n' > ${sample_id}.fasta
    echo '${params.output}/Interleaved_fasta/interleaved_reads/${sample_id}.fasta\t${sample_id}' > ${sample_id}.ksnp3_genome_list.tsv
